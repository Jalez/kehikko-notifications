 ...
 *
 * Lifted from `kehikko-journeys/wire/mailbox.ts`, which lifted it from
 * `kehikko-references`, which is where the bug below was finally understood.
 * Kept rather than adapted: two copies of a listener that behave differently
 * under the same race would be worse than two copies that behave identically.
 *
 * ## It matters more here than anywhere it has been copied to before
 *
 * Everywhere else this file has been used, what it saves is the GREETING, and
 * losing a greeting is recoverable in the sense that the host greets again on
 * the next frame load. This page also receives `roadmap.event`, and an event is
 * not resent by anybody: the protocol is explicit that delivery is best-effort
 * and one sent to a module still loading is lost. So the window between `load`
 * firing and this page being ready to listen is a window in which somebody
 * else's notification disappears with nothing anywhere recording that it did.
 *
 * The backlog closes that window for everything that arrives after the module
 * script runs, which is as early as any page can be. What it cannot close is
 * the part before the document exists at all, and that is why there is a store
 * behind this page rather than only a list in memory.
 *
