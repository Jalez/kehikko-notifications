 ...
 * for it, and a third option on a control in a 220-pixel pane is a third option
 * nobody reads. The user's words were "all, or this kehikko", and that is the
 * whole of it.
 *
 * The "this kehikko" button is never DISABLED, even when the pane does not know
 ...
 * sentence on the page instead, and the sentence is the point: this pane has
...
export function Bar({
  ...
  return (
    <div className="sticky top-0 z-10 flex min-w-0 flex-wrap items-center gap-1.5 border-b bg-background px-2.5 py-2 @[340px]/pane:px-3">
      <div className="flex shrink-0 overflow-hidden rounded-md border" role="group" aria-label="which notifications to show">
        <Button
          ...
          title="Everything this pane has been shown, from every kehikko."
          onClick={() => onScope('all')}
        >
          All
        </Button>
        <Button
          ...
          title={
            here
              ...
              : 'This pane has not been told which kehikko it is on, so this cannot be answered honestly.'
          }
      ...
      {/* The count is the first thing to go at the narrowest end: it is the
          ...
          matters. */}
      <span
        className="ml-auto hidden shrink-0 text-[0.7rem] whitespace-nowrap text-muted-foreground @[260px]/pane:inline"
        title={
          `This pane keeps the last ${keep} it has been shown. ` +
      ...
      {held > 0 && (
        <Button
          ...
          size="pane"
          variant="outline"
          className={armed ? 'ml-auto text-blocked @[260px]/pane:ml-0' : 'ml-auto text-muted-foreground @[260px]/pane:ml-0'}
          title={
            armed
              ? `Press again to discard all ${held}. Nothing resends them.`
              : 'Discard everything this pane holds. Takes two presses.'
          }
          onClick={onForget}
        >
          {armed ? 'Sure?' : 'Forget'}
