 ...
 *
 *   bun run register            # or: PORT=7911 bun run register
 *
 ...
 *
 * ## The plugin writes this file too, and that argument survives it
 *
 ...
 *
 * The ADDRESS is not a decision anybody made. Nobody chose 7910 — they chose to
 * be framed, and 7910 is a fact about where this process happened to bind, one
 ...
 * nothing: it reports this app as silent while it is running one port over, and
 * offers a Start button that would spawn a second copy. Rewriting the address
 * keeps the decision the person made TRUE. It does not make one.
 *
 * ## `dir` is written as well as `url`
 *
 * So a host that offers to START this app knows where `run.sh` lives. The url
 * alone says where to look; the directory says what to run, and a host with only
 * the first can report the app as silent and offer nothing to do about it.
 *
 ...
 * `roadmap.notifications.json` is what makes this `roadmap.notifications` — and
